...
|---|---|---|
| BL-01 | User registration | `POST /auth/register` with `role=USER` (default). Fields: `name`, `email`, `password`, `confirmPassword`. Validate email uniqueness, password match, hash with bcrypt (rounds=10), assign USER role, save user with `is_email_verified=false`, `onboardingCompleted=false`. Generate access + refresh tokens. Send email verification code (6-digit, 10 min expiry) async. **Unverified users can log in but cannot stream, like, create playlists, or access premium features until email is verified (BL-78).** Return `TokenResponse` (includes `onboardingCompleted: false`). |
...
| Code | Name | Description |
|---|---|---|
| BL-66 | Update user profile | `PATCH /users/me`. Authenticated user only. Updatable fields: `name`, `avatarUrl`. Validate name is non-empty if provided. If `avatarUrl` is provided: **Step 1** — regex must match `^https://` (http:// rejected); **Step 2** — server makes an outbound HEAD request with a 3-second timeout, response `Content-Type` must start with `image/`. On timeout or non-image Content-Type: reject `422 INVALID_AVATAR_URL`. On pass: save `avatarUrl`. **Precedence:** `avatarUrl` written by this endpoint and by BL-88 share the same field — last write wins. Return `UserResponse`. |
| BL-67 | Update artist profile | `PATCH /artists/me/profile`. ARTIST role only. Updatable fields: `stageName`, `bio`, `avatarUrl`, `socialLinks[]`. Validate `stageName` is non-empty if provided. If `avatarUrl` is provided: same two-step validation as BL-66 (https:// regex + HEAD check, 3-second timeout, `Content-Type: image/*` required). Reject `422 INVALID_AVATAR_URL` on failure. **Precedence:** shares the same `avatarUrl` field with BL-89. Return `ArtistProfileResponse`. |
| BL-88 | User avatar file upload | `POST /users/me/avatar`. Authenticated. `multipart/form-data`, field name `file`. Validation: (1) reject early if `Content-Length > 5 MB`; (2) validate MIME via magic bytes — accepted: `image/jpeg`, `image/png`, `image/webp` (rejects silently renamed files). Processing: center-crop and resize to 400×400 px using `sharp`. Store to MinIO `images` bucket at `avatar/users/{userId}.{ext}`. Uploading a new file overwrites the previous one at the same path. Set `user.avatarUrl` to the MinIO public URL. **Precedence:** shares the same `avatarUrl` field with BL-66 — last write wins. Return `UserResponse`. |
| BL-89 | Artist avatar file upload | `POST /artists/me/avatar`. ARTIST only. Same validation and processing as BL-88 — magic bytes, 5 MB max, center-crop to 400×400 px. Store to `avatar/artists/{artistId}.{ext}`. Set `artistProfile.avatarUrl` to the MinIO public URL. Shares `avatarUrl` field with BL-67. Return `ArtistProfileResponse`. |
| BL-86 | Genre onboarding | `POST /users/me/onboarding`. Authenticated. Body: `{ genreIds: string[], skipped: boolean }`. Rules: if `skipped=false`, `genreIds` must have 1–10 items and each must exist in the confirmed genre list; if `skipped=true`, `genreIds` is ignored. Either way: overwrites `user_genre_preferences` (upsert) and sets `onboardingCompleted=true`. **Idempotent** — calling again replaces previous selection. Return `UserResponse`. |
| BL-87 | Update genre preferences | `PATCH /users/me/genres`. Authenticated. Body: `{ genreIds: string[] }` — min 1, max 10 items, each must be a confirmed genre. Overwrites `user_genre_preferences`. This is the same underlying write as BL-86, exposed from profile settings so users can update their taste after onboarding. Return `UserResponse`. |

### Login & Session

| Code | Name | Description |
|---|---|---|
| BL-02 | Login | Find user by email (throw UNAUTHENTICATED if not found). Compare password with bcrypt (throw UNAUTHENTICATED if mismatch). Check account not locked (BL-43). Generate access + refresh tokens, save refresh token, create/update Session record (BL-42). Return `TokenResponse` (includes `onboardingCompleted: boolean` — client redirects to `/onboarding` if `false`). |
...
| Code | Name | Description |
|---|---|---|
| BL-09 | Song play counter | `songs.total_plays` is incremented **if and only if** `POST /songs/:id/play` is called with `secondsPlayed >= 30`. No other endpoint touches this counter — not `GET /songs/:id`, not `GET /songs/:id/stream`, not list endpoints. The old `song.listener` field is removed; `total_plays` replaces it entirely. See BL-29 for the full atomic write spec. |
...
| BL-29 | Playback history + play event endpoint | `POST /songs/:id/play` is the **single source of truth** for all play-related writes. The client fires this endpoint once per track when transitioning away (skip, song ends naturally, or app closes). Body: `{ secondsPlayed: number, skipped: boolean }`. Server executes **atomically in one transaction**: if `secondsPlayed >= 30` → `UPDATE songs SET total_plays = total_plays + 1` AND `INSERT INTO playback_history (userId, songId, playedAt, skipped=false)`; otherwise → `INSERT INTO playback_history (userId, songId, playedAt, skipped=true)` only (no `total_plays` increment). Returns `204 No Content`. **Cap at 500 rows per user** — FIFO eviction: before inserting, if the user already has 500 rows, delete the oldest one first. Cap enforced at write time in `PlaybackService`. At 500 rows, a user playing 16 songs/day fills the cap in ~30 days — making the 30-day analysis window (BL-35) reliable for all but extreme edge cases. This table feeds BL-35, BL-35B, and AI skill `analyze_listening_history`. |
...
| BL-51 | Artist analytics | `GET /artist/me/analytics` — ARTIST role only. Admins access any artist via `GET /admin/artists/:id/analytics`. Returns: **(1) All-time play counts** per song — sourced from `songs.total_plays` (BL-09). **(2) Time-bound plays (last 30 days)** — sourced from `playback_history`: `SELECT song_id, COUNT(*) FROM playback_history WHERE song_id IN (artist's songs) AND played_at >= NOW() - 30 days GROUP BY song_id`. **(3) Top 5 songs by plays in last 30 days** — same query, `ORDER BY count DESC LIMIT 5`. **(4) Per-song like counts** — sourced from `LikedSongs` playlist associations. **(5) Follower count** — sourced from `ArtistProfile.followerCount`. No `SongDailyStats` table. |
...
| Code | Name | Description |
|---|---|---|
| BL-35 | Rules-based recommendation | Recommend LIVE songs based on the user's **liked genres** (derived from liked songs and followed artists) and **non-skipped plays** from `playback_history`. Query window: `30d` (default) or `7d` — **no 90-day window** (the 500-row cap makes labels beyond 30d unreliable for active users). Fallback to globally most-played LIVE songs (`songs.total_plays DESC`) if the user has fewer than 5 non-skipped plays. For users who have hit the 500-row cap, the 30d query may cover fewer than 30 calendar days — this is acceptable because the available rows always represent the most recent plays, which is the highest-quality signal regardless. Recalculate daily via a scheduled batch job. **Storage: Hybrid Cache-Aside** — batch job writes results to a dedicated `recommendation_cache` DB table (persistent source of truth). On request: check Redis first (24h TTL); on cache miss, fetch from DB, return to user, and populate Redis. |
| BL-35A | Cold start onboarding | After successful registration, `onboardingCompleted=false` is set on the user and included in `TokenResponse`. The client redirects to `/onboarding` if this flag is `false`. The onboarding screen presents the confirmed genre list as selectable chips (min 1, max 10). On submit, `POST /users/me/onboarding` (BL-86) persists the selection to `user_genre_preferences` and sets `onboardingCompleted=true`. A "Skip for now" button calls the same endpoint with `skipped: true` — the recommendation engine falls back to global top songs until the user accumulates 5 non-skipped plays. Users can update genre preferences any time via `PATCH /users/me/genres` (BL-87) in profile settings. |
| BL-35B | Skip feedback loop | When a user skips a song before 30 seconds, set `playback_history.skipped = true` for that row via `PATCH /playback/skip`. The recommendation batch job (BL-35) excludes songs with `skipped=true` from the candidate pool for that user. |
...
| Code | Name | Description |
|---|---|---|
| BL-23 | Search | Search format: array of strings e.g. `["name~Rock", "totalPlays>1000"]`. Operators: `~` (LIKE), `>` (gt), `<` (lt). Combined with AND. Build TypeORM QueryBuilder dynamically. **Searches across four entities: songs, albums, artists, and playlists.** Search only returns LIVE songs — SCHEDULED songs excluded from song results. |
| BL-24 | Pagination | Default: `page=1`, `size=20`. All list endpoints return: `page`, `size`, `totalPages`, `totalItems`, `items[]`. |
...
| BL-09 | Song play counter | 4. Content Management |
