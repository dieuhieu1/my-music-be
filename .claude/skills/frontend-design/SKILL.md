...
This skill guides creation of **Midnight Vinyl**-consistent, animation-rich, production-grade
interfaces for the My Music app. Every output must be visually striking, responsive, and
tightly integrated with the existing design system — not a generic music UI template.
...
```tsx
// Parent: no animation class
// Each child:
<div className={`anim-fade-up anim-fade-up-${Math.min(i + 1, 8)}`}>
```
Cap at `anim-fade-up-8` for long lists — first 8 animate, rest appear static.
...
**WaveBar visualizer recipe** (5 bars, compose inline):
```tsx
...
```tsx
// Only activate when text overflows — measure with useRef
const duration = (innerWidth / 50).toFixed(1); // 50px per second
...
|------------------|------------------------|----------------------------------------------------|
| `follow-bounce`  | `onClick` via setState | Follow/unfollow button click (see FollowButton.tsx)|
...
For album pages, tint the ambient layer with the album's dominant color (pass as prop).

### Vinyl Disc Component (decorative / loading)

```tsx
// Loading state — prefer over bare <Loader2> spinner
<div
...
className="tagIn"  // entrance animation
```
...
style={{
  ...
  textTransform: 'uppercase',
  color: 'rgba(232,184,75,0.35)',  // nav context
  // OR: color: 'var(--gold)'      // page context
...
```tsx
// 3 absolutely-positioned divs, zIndex: -1, pointerEvents: 'none'
const orbs = [
...
// Each orb: position absolute, borderRadius 50%, filter blur(80px), mixBlendMode screen
```

### Dividers

```tsx
// Gradient (hero → content transition)
...
```
Reserve `useState` hover only when child elements also need to react.

### Transition Standard
```tsx
// State changes (color, border, opacity)
transition: 'color 0.18s, background 0.18s, border-color 0.18s'

// Transforms (spring-like)
transition: 'transform 0.25s cubic-bezier(0.16,1,0.3,1)'
```
`cubic-bezier(0.16,1,0.3,1)` is the project's standard spring easing — use it everywhere.

### Stagger Formula (programmatic)
```tsx
animationDelay: `${i * 0.06}s`  // mirrors anim-fade-up-1..8 step increments
```
...
### Browse / Discover
Discovery + serendipity. Dark grid where album art dominates.
- Hero: horizontally scrollable genre pill row (marquee on overflow)
- Featured: large glassmorphism card with aurora background + `beamSweep` on hover
- Grid: staggered `anim-fade-up` on album cards, hover lifts with `translateY(-4px)` + gold border brightens
- Sections ("New Releases", "Trending"): horizontal scroll carousels with section label headers

### Activity Feed
Social, live, real-time. Record store bulletin board feel.
...
- Timeline: thin gold vertical line on left edge, `goldLine` animated in on mount

### Playlist Detail
Curated collection. Editorial, magazine-adjacent.
...
- Gradient fade at tracklist bottom bleeding into PlayerBar

### Album Detail
Premium vinyl-unboxing experience.
...
- Release year, label, genre pills in muted-text caps style

### Artist Public Profile (extend existing)
Already well-implemented. Phase 5 additions:
- Songs section: staggered song rows, waveBar on playing track
- Discography grid: album cards with hover `vinyl-glow`

### PlayerBar — Full Implementation
The most critical component. Must feel like a vinyl record being played.
- Circular album art + `vinyl-spin vinyl-glow` when playing
- waveBar visualizer (5 bars) replaces the current text "▶ Playing"
- Gold scrubbing progress bar (3px height)
- Frosted glass container (`backdropFilter: blur(16px)`)
...
- Never use white `#ffffff` for text → use `var(--ivory)` or `rgba(245,238,216,0.7)`

### Color
- Never use Tailwind's default gray palette (`gray-700`, `gray-900`, etc.)
- Never use purple gradients or blue accents
- Never use fully opaque gold as a background fill — only for text and thin borders
- Never use `bg-white` or `text-black` anywhere

### Animation
- Never import Framer Motion, `@motionone/react`, or GSAP
- Never write `@keyframes` inside component files → all keyframes live in `globals.css`
...
- Never stack more than 3 simultaneous ambient animations on one element

### Structure
- Never use Tailwind grid/flex for complex layouts → use inline styles
- Never create a new CSS file → extend `globals.css`
- Never use Radix UI outside the approved list (Dialog, Dropdown, Toast, Avatar)
- Never place PlayerBar outside the fixed-bottom layout

### Loading States
- Never use a bare `<Loader2>` spinner for full-page loads → use vinyl-spin disc variant
- Use `.shimmer` class (already in `globals.css`) if shimmer is needed, never reinvent it
...
- **Every animation must serve a purpose**: playback feedback, navigation clarity, or atmosphere. No animation for its own sake.
- **Playfair Display numbers are a visual signature** — use them for all stats, counts, and timestamps.
- **Gold is earned** — use it sparingly as accent, not as fill. It loses power when overused.
