...
export default function UploadSongModal({ open, onOpenChange, artist, onSuccess }: Props) {
  ...
  const removeCoverArt = () => {
    if (coverPreviewUrl && coverInputMode === 'file') URL.revokeObjectURL(coverPreviewUrl);
    setCoverPreviewUrl(null);
    setCoverArtFile(null);
    setCoverArtUrlInput('');
  };
  ...
  const handleUpload = async () => {
    ...
    if (coverInputMode === 'file' && coverArtFile) {
      formData.append('coverArt', coverArtFile);
    } else if (coverInputMode === 'url' && coverArtUrlInput.trim()) {
      formData.append('coverArtUrl', coverArtUrlInput.trim());
    }
  ...
  const isCoverUrlValid = coverInputMode === 'file' || !coverArtUrlInput.trim() || !!coverPreviewUrl;
  const canSubmit = !!(title.trim() && audioFile && selectedGenreIds.length > 0 && !uploading && isCoverUrlValid);
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>
                    Cover Art <span style={{ fontSize: 12, color: 'var(--text-faint)', fontWeight: 400 }}>(optional)</span>
                  </label>
                  <div style={{ display: 'flex', gap: 4, background: 'var(--surface-raised)', padding: 3, borderRadius: 6, border: '1px solid var(--border)' }}>
                    <button
                      ...
                      style={{
                        ...
                        background: coverInputMode === 'file' ? 'var(--accent)' : 'transparent',
                        color: coverInputMode === 'file' ? '#fff' : 'var(--text-muted)',
                        transition: 'all 150ms'
                      }}
                    >File</button>
                    <button
                      ...
                      style={{
                        padding: '4px 10px', fontSize: 12, borderRadius: 4, cursor: 'pointer', border: 'none',
                        background: coverInputMode === 'url' ? 'var(--accent)' : 'transparent',
                        color: coverInputMode === 'url' ? '#fff' : 'var(--text-muted)',
                        transition: 'all 150ms'
                      }}
                    >URL</button>
                  </div>
                      ...
                      <img
                        ...
                        onError={() => {
                          if (coverInputMode === 'url') {
                            setCoverPreviewUrl(null); // Invalid image URL, hide preview
                          }
                        }}
                        style={{ width: coverInputMode === 'url' ? 40 : 64, height: coverInputMode === 'url' ? 40 : 64, objectFit: 'cover', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}
                      />
                      <button
