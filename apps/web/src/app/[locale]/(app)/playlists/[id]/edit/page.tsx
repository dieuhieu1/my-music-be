...
export default function EditPlaylistPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/${locale}/playlists/${id}`);
    } catch (err: any) {
  ...
  if (!playlist || (user && playlist.userId !== user.id)) {
    return <div style={{ padding: 40, color: 'var(--muted-text)' }}>Playlist not found or access denied.</div>;
  }
      ...
      <div className="anim-fade-up" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 32 }}>
          ...
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 400, color: 'var(--ivory)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 280 }}>
              {playlist.title}
            </h1>
          </div>
        </div>
