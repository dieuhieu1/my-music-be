...
| # | Screen | Access | Key Function | BL Codes |
|---|---|---|---|---|
| C1 | Public Artist Profile | Public (no JWT required) | stageName, bio, avatarUrl, followerCount, socialLinks. Shows **LIVE songs only**. Follow / Unfollow button (self-follow forbidden). **`artist.followerCount` increments on every `GET /artists/:id` request (BL-11).** | BL-47, BL-11 |
